...
try {
	...
	if ($existant_answer) {
		...
		$upd = good_query("
		UPDATE SurveyAnswers SET Answer = '$Answer', ztamp = NOW() WHERE AnswerID = $existant_answer
		");
	
	} else {
	
		$ins = good_query("
		INSERT INTO SurveyAnswers (QuestionID, WineryID, UserID, Answer, ztamp)
		VALUES ($QuestionID,$WineryID,$UserID,'$Answer',NOW())
		");
