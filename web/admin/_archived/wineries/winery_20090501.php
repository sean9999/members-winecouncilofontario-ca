...
<?php
require_once INC . 'function.listFilesIn.php';
require_once INC . 'function.characterConversions.php';
$path2Images = HomePathLocal.WineryImagesPath.LegalizeText(good_query_value("SELECT Name FROM Regions WHERE RegionID = $RegionID"));
...
<tr>
	...
	if ($wine['WineID'] == $FeaturedWineID)	$extra_HTML = ' selected="selected"';
	else									$extra_HTML = '';
	echo '<option value="'.$wine['WineID'].'"'.$extra_HTML.'>'.$wine['Name'].'</option>';
	}
	?>
	</select>
	</td>
