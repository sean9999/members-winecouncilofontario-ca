...
<div class="pretty_table_wrapper">
	...
	function drillDescendants($ChunkID=0,$generation=0) {
		...
		$p = array('fields' => 'ChunkID,ChildOf,Name,Title,SEOName');
	...
	function spitRow($r,$generation) {
		...
		<tr class="chunkrow childof_{$r['ChildOf']}">
			<td>{$pad}{$pad2}</td>
			<td><a href="?view=chunk&amp;ChunkID={$r['ChunkID']}">{$pad3}{$r['Name']}</a></td>
			<td align="right">
...
<div class="pretty_table_wrapper">
	...
	<table class="pretty">
		...
		echo '	<td><a href="?view=chunk&amp;ChunkID='.$p['ChunkID'].'">'.$p['Name'] . '</a></td>';
		echo '	<td align="right">
				<ul class="chunk_actions smalltext">
