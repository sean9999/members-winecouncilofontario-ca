...
$section_id		= 'account';
$section_title	= 'Apply for an Account';
instantiate_header();
$header -> body_id = 'home';
...
if(isset($_POST['FormAccount']))
{
	...
	{ 
		...
		$mailbody = 'A new user has applied to the system to members.winecouncilofontario.ca. ' . $P['name'] . ' - ' . $P['email'];		
		$crlf	= "\n";
		...
		if ($P['wineryID'] == 99999) {
		    $UserLevel = 'Trade';
		} elseif ($P['wineryID'] == 88888) {
		    $UserLevel = 'Non-Member';
		} else {
		    $UserLevel = 'Employee';
		}
		good_query("INSERT INTO Users (Name, Title, Email, Password, WineryID, Company, Notes, UserLevel, ztamp) VALUES ('" . $P['name'] . "', '" . $P['title'] . "', '" . $P['email'] . "', '" . $P['password'] . "', " . $P['wineryID'] . ", '" . $P['Company'] . "', '" . $P['notes'] . "', '" . $UserLevel . "', NOW())");
		echo "<script>console.log('".EMAIL_RECEIVER."', '".EMAIL_DEV."'); window.location = 'account-create-thanks.php'</script>";
	}
}
...
<table class="form">
	...
	<tr>
		...
		<td class="right">
			<input type="email" name="email" id="#" value="" width="50" />
		</td>
	</tr>	
	<tr>
		<td class="left">
	...
	<tr>
		...
		<td class="right">
			<?php
			connect2database ();
			$wineries = good_query_table("SELECT Name, WineryID, IsMember FROM Wineries ORDER BY Name" );
			//$wineries = good_query_table("SELECT * FROM Wineries" ); 
			//  || ($w['WineryID'] != 300)
			?>
			<select name="wineryID">
				<option value=""> - Select Your Winery / Company - </option>
				<option value="99999">* Supplier / Non-Winery</option>
				<option value="88888">* My Winery is not listed</option>
				<?php
				foreach ($wineries as $w) {
					if (($w['WineryID'] != '302') && ($w['WineryID'] != '300')) {
						echo '<option value="';
						echo $w['WineryID'];
	...
	<td class="left">
		...
		<td class="right">
			<input type="text" name="Company" id="#" value="" width="50" /><br />
			<em>If you are a supplier or if your winery is not in the list, please fill in your company name above.</em>
		</td>
	</tr>	
